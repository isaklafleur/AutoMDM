...
db.eclasses.drop();
db.getCollection('eclasses').find({}).sort( { codeName: 1 } );
...
query.exec(function (err, completeeclasscode) {
    ...
    console.log('%s %s %s %s', eClass.eClassSegment, eClass.eClassMainGroup, eClass.eClassGroup, eClass.CommodityClass);
});