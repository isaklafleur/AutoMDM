...
db.once('open', () => {
  const getOneeClassCode = function () {
  ...
  const getAlleClassCodes = function () {
  ...
  const getAlleClassCodesLoop = function () {
    EClass.find({}, (err, result) => {
      if (err) {
        console.log('Error: ', err);
      }
      result.forEach((el) => {
        // I have the result and need to interate over it to
        // create the object with parent and children.
